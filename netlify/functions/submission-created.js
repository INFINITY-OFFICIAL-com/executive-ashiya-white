...
exports.handler = async function (event) {
  ...
  const age = payload.data['年齢層'] || '';
  const occupation = payload.data['ご職業・役職'] || '';
  const company = payload.data['会社名'] || '';
  const email = payload.data['メールアドレス'] || '';
  const phone = payload.data['電話番号'] || '';
  const area = payload.data['居住エリア'] || '';
  const health = payload.data['健康習慣'] || '';
  const referrer = payload.data['ご紹介者名'] || '';
  const referrerRelation = payload.data['ご紹介者との関係'] || '';
  const motivation = payload.data['ご入会の動機・ご要望'] || '';
  ...
  <div style="max-width:600px;margin:0 auto;font-family:'Noto Serif JP','Yu Mincho',Georgia,serif;color:#1C1F2E;line-height:2.2;">
    ...
    <div style="padding:2.5rem 2rem;">
      ...
      <p style="font-size:13px;color:rgba(28,31,46,0.68);line-height:2.4;">
        ...
        お預かりしたご登録内容につきまして、弊社にて確認させていただきます。<br><br>
...
<body style="margin:0;padding:2rem;font-family:sans-serif;color:#1C1F2E;">
  <h2 style="font-size:18px;margin-bottom:1.5rem;color:#8B6F1F;">新規ウェイティングリスト登録</h2>
  <table style="border-collapse:collapse;width:100%;max-width:600px;font-size:14px;">
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;width:160px;">氏名</td>
    ...
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;">年齢層</td>
      <td style="padding:10px 14px;border:1px solid #ddd;">${age}</td>
    </tr>
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;">ご職業・役職</td>
      <td style="padding:10px 14px;border:1px solid #ddd;">${occupation}</td>
    </tr>
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;">会社名</td>
      <td style="padding:10px 14px;border:1px solid #ddd;">${company || '（未記入）'}</td>
    </tr>
    <tr>
    ...
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;">現在の健康習慣</td>
      <td style="padding:10px 14px;border:1px solid #ddd;">${health}</td>
    </tr>
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;">ご紹介者名</td>
      <td style="padding:10px 14px;border:1px solid #ddd;">${referrer || '（なし）'}</td>
    </tr>
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;">ご紹介者との関係</td>
      <td style="padding:10px 14px;border:1px solid #ddd;">${referrerRelation || '（未選択）'}</td>
    </tr>
    <tr>
      <td style="padding:10px 14px;border:1px solid #ddd;font-weight:bold;background:#f9f9f7;">ご入会の動機・ご要望</td>
      <td style="padding:10px 14px;border:1px solid #ddd;white-space:pre-wrap;">${motivation || '（未記入）'}</td>
    </tr>
  </table>
  ...
  try {
    ...
    const replyRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: '【Executive Ashiya】ウェイティングリスト登録受付のご連絡',
        html: autoReplyHtml,
      }),
    });
    ...
    const notifRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: `${FROM_NAME} <${FROM_EMAIL}>`,
        to: [FROM_EMAIL],
        subject: `【EA新規登録】${name} 様 / ${area} / ${occupation}`,
